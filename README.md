...
-----
해당 웹사이틍 구현할 당시 ,
Update기능과 Delete 기능 구현 당시 DB에서 정보를
1. 어떻게 불러올지
2. 불러온다면 어떻게 수정을 하고 어떻게 다시 전달할지
3. 만약 삭제한다면 어떻게 삭제할 것인지  
가 제일 힘든 부분이었습니다.  
우선 단계적 해결을 위해 불러오는 방법부터 찾았었는데 이전 상세보기 페이지에서
DB에 저장된 상품의 고유 ID값을 통해서 상품 정보를 불러왔던 점을 이용하여 
